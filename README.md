...
当前主线版本：`v2.4.0 C# / .NET 8 WinForms`。
...
- 窗口为紧凑尺寸，不默认占满屏幕
- 表格列宽支持手动拖动，自动保存并在下次启动恢复
- 表格内容进一步压缩，减少省略号
- 日志区高度增高，初始连接信息不用横向滑动查看
...
- 固定最新快照文件：`latest_snapshot.json`，给 Excel/管理软件读取当前最新状态
- 自动清理旧构建/临时发布文件
- 数据保留天数限制为 7–30 天
...
`DATA_OUTPUT_DIR` 留空表示程序所在目录；填写路径后，会在该目录下生成；运行中切换输出目录会自动初始化新目录数据库：
